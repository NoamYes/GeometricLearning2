...
# Z_reduced_diffusion = Diffusion_Maps(Z, kernel_method='Gaussian', epsilon=1, d=3)
...
s_reduced_diffusion = Diffusion_Maps(s.T, n_neighbor=10, kernel_method='Polynomial', normalized=False, epsilon=1, d=2,
                                     **kwargs)
...
s_reduced_LLE = Locally_Linear_Embedding(s.T, n_neighbor=10, epsilon=1, d=2)
...
for i, n_neighbor in enumerate(neighbors_list):
    s_reduced_diffusion = Diffusion_Maps(s.T, n_neighbor=n_neighbor, kernel_method='Polynomial', epsilon=1, d=2,
                                         **kwargs)
...
for i, kernel_method in enumerate(kernels_list):
    ...
    s_reduced_diffusion = Diffusion_Maps(s.T, n_neighbor=n_neighbor, kernel_method=kernel_method, epsilon=1, d=2,
                                         **kwargs)

    scat = axs[i].scatter(s_reduced_diffusion[:, 0], s_reduced_diffusion[:, 1], c=colormap)
    axs[i].set_title(str(kernel_method) + kargs_string)

title = 'Torus Diffusion MAP for multiple kernel methods, n_neighbors=' + str(
...
axs = axs.ravel()
for i, n_neighbor in enumerate(neighbors_list):
    s_reduced_LLE = Locally_Linear_Embedding(s.T, n_neighbor=n_neighbor, epsilon=1, d=2)
...
for n_class in classes:
    ...
    for i, n_neighbor in enumerate(neighbors_list):
        digits_reduced_diffusion = Diffusion_Maps(digits_data, n_neighbor=n_neighbor, kernel_method='Linear', epsilon=1,
                                                  d=2)
    ...
    kernels_list = ['Linear', 'Gaussian', {'a': 1, 'b': 0, 'c': 0.5}, {'a': 1, 'b': 0, 'c': 2},
                    {'a': 1, 'b': 0, 'c': 4}]
    n_neighbor = 10
    colormap = digits_tags
    ...
    for i, kernel_method in enumerate(kernels_list):
        ...
        if not type(kernel_method) == str:
            ...
            a, b, c = kwargs['a'], kwargs['b'], kwargs['c']
            kargs_string = '  a =' + str(a) + ' b=' + str(b) + ' c=' + str(c)
        digits_reduced_diffusion = Diffusion_Maps(digits_data, n_neighbor=n_neighbor, kernel_method=kernel_method,
                                                  epsilon=1, d=2, **kwargs)

        scat = axs[i].scatter(digits_reduced_diffusion[:, 0], digits_reduced_diffusion[:, 1], c=colormap)
        axs[i].legend(handles=scat.legend_elements()[0], labels=[str(v) for v in digits_tags])
        axs[i].set_title(str(kernel_method) + kargs_string)

    title = 'Digits Diffusion MAP for multiple kernel methods, n_neighbors=' + str(
        n_neighbor) + '\n' + 'Polynomial model=' + r'$a(xx^T+b)^c$'
    fig.suptitle(title)
    # plt.show()
...
for n_class in classes:
    ...
    for i, n_neighbor in enumerate(neighbors_list):
        digits_reduced_LLE = Locally_Linear_Embedding(digits_data, n_neighbor=n_neighbor, epsilon=1, d=2)
